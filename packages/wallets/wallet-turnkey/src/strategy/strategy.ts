  ...
  implements ConcreteWalletStrategy
{
  ...
  public static async create(args: TurnkeyArgs): Promise<TurnkeyWallet> {
    ...
    const localToken = wallet.getItem(TURNKEY_TOKEN_KEYS.TOKEN)
    const localOrganizationId = wallet.getItem(
      TURNKEY_TOKEN_KEYS.ORGANIZATION_ID,
    )
    // TODO
    // const localTokenExpiration = wallet.getItem(TURNKEY_TOKEN_KEYS.TOKEN_EXPIRATION)

    if (localToken && localOrganizationId) {
      wallet.organizationId = localOrganizationId
      await wallet
        ...
        .catch((e) => {
          if (e.code === 7) {
            wallet.removeItem(TURNKEY_TOKEN_KEYS.TOKEN)
            wallet.removeItem(TURNKEY_TOKEN_KEYS.ORGANIZATION_ID)
            wallet.setStatus('init')
          } else {
            throw e
          }
        })
  ...
  private async initializeIframe(): Promise<void> {
    ...
    if (existingIframeClient) {
      // TODO: do we have to remove this or can we reuse it?
  ...
  // TODO: allow configurable storage
  private setItem(key: TurnkeyTokenValue, value: string) {
    localStorage.setItem(key, value)
  }

  private getItem(key: TurnkeyTokenValue) {
    return localStorage.getItem(key)
  }

  private removeItem(key: TurnkeyTokenValue) {
  ...
  async initEmailOTP(args: {
    ...
    try {
      ...
      this.setItem(TURNKEY_TOKEN_KEYS.ORGANIZATION_ID, organizationId)
  ...
  private async loadClientAndGetWallets(args: {
    token: string
    organizationId?: string
  }) {
    const { token } = args
    ...
    try {
      ...
      if (loginResult) {
        ...
        this.setItem(TURNKEY_TOKEN_KEYS.TOKEN, token)
        this.setItem(TURNKEY_TOKEN_KEYS.ORGANIZATION_ID, organizationId || '')
        // TODO
        // this.setItem(TURNKEY_TOKEN_KEYS.TOKEN_EXPIRATION, loginResult.expiration.toISOString())
      }
    } catch (e) {
      throw new WalletException(new Error((e as any).message), {
        code: UnspecifiedErrorCode,
  ...
  async getAddresses(): Promise<string[]> {
    try {
      ...
      const { wallets } = await this.authIframeClient
        ...
        .catch((e) => {
          if (e.code === 7) {
            this.removeItem(TURNKEY_TOKEN_KEYS.TOKEN)
            this.removeItem(TURNKEY_TOKEN_KEYS.ORGANIZATION_ID)
            this.setStatus('init')
            throw new WalletException(new Error('User is not logged in'), {
              code: UnspecifiedErrorCode,
              type: ErrorType.WalletError,
      ...
      const fetchAccountsPromises = wallets.map((wallet) =>
      ...
      const allWalletAccountsSettled = await Promise.allSettled(
        fetchAccountsPromises,
      )
      const allWalletAccounts = allWalletAccountsSettled
        .filter((result) => result.status === 'fulfilled')
        .flatMap((result) => result.value?.accounts)

      const accountAddresses = allWalletAccounts.filter(
      ...
      this.accountAddresses = accountAddresses.map((wa) => wa.address)
